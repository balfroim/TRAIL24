...
def train(args):
    ...
    if args.new_user:
        pretrained_file = '{}/policy_model_epoch_50.ckpt'.format(args.log_dir)
        pretrain_sd = torch.load(pretrained_file)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.new_user:
            policy_file = '{}/policy_model_new_user.ckpt'.format(args.log_dir, epoch)
            logger.info("Save models to " + policy_file)
            torch.save(model.state_dict(), policy_file)
