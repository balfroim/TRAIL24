...
def train(args):
    ...
    if args.new_user == True:
        state_dict = torch.load('{}/transe_model_sd_epoch_1.ckpt'.format(args.log_dir))
...
def extract_embeddings(args, dataset):
    ...
    model_file = '{}/transe_model_sd_epoch_{}.ckpt'.format(args.log_dir, 1)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default=ML1M, help=f'One of [{ML1M}, {LFM1M}, beauty, cd, cell, clothing]')
    ...
    parser.add_argument('--new_user', type=bool, default=True, help='Allow prediction on new user')
